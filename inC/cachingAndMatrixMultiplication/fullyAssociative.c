...
void accessData (
    ...
    if ( cache->occupancy == E ) {
        ...
        /* ... */
        curr_line = cache->front;
        while (curr_line->next_cache_line->next_cache_line !=NULL){
            curr_line = curr_line->next_cache_line;
        }
        cache_line_t* temp = cache->back;
        curr_line->next_cache_line = NULL;
        cache->back = curr_line;
        (*eviction_count)++;
        cache->occupancy--;
        free(temp);
    }
    ...
    /* ... */
    cache_line_t* newFront = calloc(1,sizeof(cache_line_t));
    newFront->tag = tag;
    if (cache->occupancy == 0){
        cache->front = newFront;
        cache->back = newFront;
        cache->occupancy++;
    }
    else {
    newFront->next_cache_line = cache->front;
    cache->front = newFront;
    cache->occupancy++;
    }
